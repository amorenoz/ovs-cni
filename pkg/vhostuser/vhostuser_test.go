...
// ovsSocketFile returns the OVS DB socket path, honoring OVS_RUNDIR.
func ovsSocketFile() string {
	runDir := os.Getenv("OVS_RUNDIR")
	if runDir != "" {
		return fmt.Sprintf("unix:%s/db.sock", runDir)
	}
	return ""
}
	...
	Context("with OVS bridge", func() {
		...
		Context("ADD with VLAN tag", func() {
			It("should create a port with the correct VLAN tag", func() {
				...
				devInfoFile := writeDevInfoFile(tmpDir, "client", socketPath)
				socketFileEntry := ""
				if sf := ovsSocketFile(); sf != "" {
					socketFileEntry = fmt.Sprintf(`"socket_file": "%s",`, sf)
				}
				conf := fmt.Sprintf(`{
					"cniVersion": "1.0.0",
					"name": "mynet",
				...
				}`, bridgeName, socketFileEntry, devInfoFile)
		...
		Context("ADD returns empty result", func() {
			It("should return a result with no interfaces and no IPs", func() {
				tmpDir := GinkgoT().TempDir()
				socketPath := filepath.Join(tmpDir, "sock-dir", "vhost.sock")
				devInfoFile := writeDevInfoFile(tmpDir, "client", socketPath)
				conf := vhostUserConf("1.0.0", bridgeName, devInfoFile)
				...
				Expect(err).NotTo(HaveOccurred())
				Expect(result.Interfaces).To(BeEmpty())
...
func vhostUserConf(version, bridge, devInfoFile string) string {
	...
	}`, version, bridge, socketFileEntry, devInfoFile)
}
